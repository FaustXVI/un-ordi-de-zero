...
class LoiNoeud(MyScene):
 ...
 et nécessaire dans mon explication de la loi des mailles.
                """) as timer:
            electrons = [Dot(color=YELLOW) for i in range(0, timer.duration * 4)]
        ...
        with self.my_voiceover(
            ...
            self.play(TransformMatchingTex(simpleSum, oppositeSum))
        flippedSum = MathTex("i_1", "+", "i_2", "+ ", "i_3", "+ ", "i_4", "=", "0")
        with self.my_voiceover(
                """Si on passe i3 et i4 de l'autre côté, on obtient""") as timer:
            self.play(TransformMatchingTex(oppositeSum, flippedSum))
        ...
        with self.my_voiceover(
            ...
            self.play(TransformMatchingTex(flippedSum, finalSum, key_map={"i_4": "i_n"}))
        self.next_section(skip_animations=False)
        self.wait(3)
